...
class PolicyTest extends TestDriver {
    ...
    public static void main(String argv[]) {
        ...
        System.out.println("\n" + "Running " + TEST_NAME + "...");
        boolean status = PASS;
        status &= bmt.mruFavorableTest();
        
        bmt = new PolicyTest();
        bmt.create_minibase();
        status &= bmt.multipleAccesses();
     ...
     * 
     */
    protected boolean mruFavorableTest() {
        ...
        for (pid.pid = firstPid.pid; status == PASS && pid.pid < firstPid.pid+partBufPages; pid.pid = pid.pid + 1) {
            try {
            ...
            for (pid2.pid = firstPid.pid + partBufPages; status == PASS && pid2.pid < firstPid.pid+numDiskPages; pid2.pid = pid2.pid + 1) {
                try {
    ...
    protected boolean multipleAccesses() {  // testing all policies thru random sequence of page 
