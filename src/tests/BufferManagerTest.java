...
class BufferManagerTest extends TestDriver {
    ...
    public static void main(String argv[]) {
        ...
        System.out.println("\n" + "Running " + TEST_NAME + "...");
        boolean status = PASS;
        status &= bmt.bufferTest();
        status &= bmt.illegalBufferTest();
        status &= bmt.bufferManagerInternalsTest();
    ...
    protected boolean bufferTest() {
        ...
        for (pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages; status == PASS && pid.pid < lastPid.pid; pid.pid = pid.pid + 1) {
     ...
     * 
     */
    protected boolean illegalBufferTest() {
        ...
        for (pid.pid = firstPid.pid + 1, lastPid.pid = firstPid.pid + numPages - 1; status == PASS
          ...
          // System.out.println(Minibase.BufferManager.getNumUnpinned());
     ...
     * 
     */
    protected boolean bufferManagerInternalsTest() {
